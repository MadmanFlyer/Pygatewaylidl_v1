...
def DecodeAES(kek_str = "", encrypted_AUSKEY = ["",""]):
    ...
    kek = _decode_kek(_get_bytes(kek_str))#input("Enter KEK hex string line>")))

    encoded_key = b""
    for n in range(2):
        a = encrypted_AUSKEY[n] #input("Encoded aus-key as hex string line %d>" % (n+1))
    ...
    Auskey_decode = []
    Auskey_decode[0] = auskey.decode("ascii")
    Auskey_decode[1] = auskey[-8:].decode("ascii")

    return Auskey_decode
